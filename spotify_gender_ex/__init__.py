...
def start_genderex(apk_file='', directory='.', replacement_table='', ks_password='', key_password='', ignore_ssl=False,
    ...
    gex = genderex.GenderEx(apk_file, directory, replacement_table, no_interaction, debug, ks_password, key_password,
                            ignore_ssl, no_logfile)
...
def run(a, d, rt, kspw, kypw, nossl, noia, force, cleanup, debug):
    """Entferne die Gendersternchen (z.B. Künstler*innen) aus der Spotify-App für Android!"""
    start_genderex(a, d, rt, kspw, kypw, nossl, noia, force, cleanup, debug)
